...
//        dd($dest_dir,$event,$cpId,$cpVersion,$temp_test_dir, $dest_test_dir);
        ...
        $verifierId1 = ApprRecord::select("approver")->where("appr_event",$event->id)->get()[0]["approver"];
		$verifierId2 = ApprRecord::select("approver")->where("appr_event",$event->id)->get()[1]["approver"];
		$verifier_org_id1 = User::select("group_id")->where("id",$verifierId1)->get()[0]["group_id"];
		$verifier_org_id2 = User::select("group_id")->where("id",$verifierId2)->get()[0]["group_id"];
		$verifier_org_name1 = Group::select("group_name")->where("id",$verifier_org_id1)->get()[0]["group_name"];
		$verifier_org_name2 = Group::select("group_name")->where("id",$verifier_org_id2)->get()[0]["group_name"];
        $testId = Event::select("user_id")->where("id",$testEvent->id)->get()[0]["user_id"];
        $testOrg_id = User::select("group_id")->where("id",$testId)->get()[0]["group_id"];
        $testOrg = Group::select("group_name")->where("id",$testOrg_id)->get()[0]["group_name"];
        $developerId = Event::select("user_id")->where("id",$devEvent->id)->get()[0]["user_id"];
        $developerOrg_id = User::select("group_id")->where("id",$developerId)->get()[0]["group_id"];
        $developerOrg = Group::select("group_name")->where("id",$developerOrg_id)->get()[0]["group_name"];
		$module_org_id = Component::select("group_id")->where("id",$event->comp_id)->get()[0]["group_id"];
		$module_Org_name = Group::select("group_name")->where("id",$module_org_id)->get()[0]["group_name"];
        $storage_arr = array(
            "moduleId"=>$event->comp_id,
            "moduleOrg"=>$module_Org_name,
            "moduleFinger"=>$filr_fingerprint,
            "moduleName"=>$comp_name,
            "moduleVersion"=>$event->ver_num,
            "productName"=>"产品名",
            "testReportFinger"=>$testDoc_fingerprint,
            "testerOrg"=>$testOrg,
            "developerOrg"=>$developerOrg,
            "verifierOrg1"=>$verifier_org_name1,
            "verifierOrg2"=>$verifier_org_name2,
            "sore"=>"95"
        );

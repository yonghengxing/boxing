			...
			<div class="widget am-cf">
				...
				<div class="widget-body am-fr">
					<form class="am-form tpl-form-border-form  tpl-form-border-br"
						...
						<div class="am-form-group">
							<label class="am-u-sm-3 am-form-label">选择审批类型 <span
								class="tpl-form-line-small-title"> Type</span></label>
							<div class="am-u-sm-9">
								<select data-am-selected="{searchBox: 1}" style="display: none;"
									name="action_type" id="action_type">
									@foreach(config('app.action_type') as $id=> $type) @if($id ==
									$select_action)
									<option name="type" value="{{ $id }}" selected="selected">{{
										$type }}</option> @else
									<option name="type" value="{{ $id }}">{{ $type }}</option>
										...
										<tr class="gradeX">
											...
											<td>
												<div class="tpl-table-black-operation">
													<a href="{{asset('/moban/node')}}/{{$node->id}}"> <i
							...
							<div class="am-u-sm-9">
								...
								<script> 
									function del(){
										if(confirm("确定要删除吗？")){
											alert('删除成功！');
											return window.location.href="{{asset('/moban/delete')}}/{{$select_type}}/{{$select_action}}";
										}else{
											return false; 
										}
									} 
						...
						<div class="widget-body am-fr">
							<div class="am-form-group" id="pre_app">
								...
								<div
									...
									<a href="{{asset('/moban/new')}}/{{$select_type}}/{{$select_action}}/1"
				...
				<script type="text/javascript">
					$(function(){
						$("#comp_type").change(function(){
							var value = $(this).val();
							var action_val = document.getElementById("action_type").value;
							var str1 = "{{asset('moban/show')}}";
							var url = str1 + '/' + value + '/' + action_val;

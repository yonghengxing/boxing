			...
			<div class="widget am-cf">
				...
				<div class="widget-body  am-fr">
					<div class="am-u-sm-9">
						...
						<select data-am-selected="{searchBox: 1}" style="display: none;"
							name="import_able" id="import_able">
    						 @foreach(config('app.import_status') as $id=> $type)
    						 	@if($id == $import_status)
    								<option name="type" value="{{ $id }}" selected="selected">{{$type }}</option>
    							@else
    								<option name="type" value="{{ $id }}">{{ $type }}</option>
						...
						<table width="100%"
							...
							<tbody>
								...
								<tr class="gradeX">
									...
									<td>{{ config("app.comp_status")[$component->dev_import] }}</td>
									<td>
										<div class="tpl-table-black-operation">
											@if($component->dev_import == 1001) <a
												href="{{ asset('/developer/import')}}/{{$component->id}}"> <i
												class="am-icon-pencil"></i> 入库
											</a> @endif
										</div>
									</td>
								</tr>
					...
					<script type="text/javascript">
    					$(function(){
    						$("#comp_type").change(function(){
    							...
    							var import_val = document.getElementById("import_able").value;
    							var value = $(this).val();
    							var str1 = "{{asset('developer/complist')}}";
    							var url = str1 + '/' + value + '/' + group_val + '/' + import_val;
    						...
    						$("#group_choose").change(function(){
    							...
    							var value = $(this).val();
    							var str1 = "{{asset('developer/complist')}}";
    							var url = str1 + '/' + comp_val + '/' + value + '/' + import_val;
    							window.location.href= url;
    						});
    						$("#import_able").change(function(){
    							var comp_val = document.getElementById("comp_type").value;
    							var group_val = document.getElementById("group_choose").value;
    							var value = $(this).val();
    							var str1 = "{{asset('developer/complist')}}";
    							var url = str1 + '/' + comp_val + '/' + group_val + '/' + value;
    							window.location.href= url;
    						});
    					});
					</script>
